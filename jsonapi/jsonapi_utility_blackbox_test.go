...
func TestErrorToJSONAPIError(t *testing.T) {
	...
	jerr, httpStatus = jsonapi.ErrorToJSONAPIError(nil, errors.NewTenantRecordNotFoundError("entity", "ID"))
	...
	// test tenant not found error
	jerr, httpStatus = jsonapi.ErrorToJSONAPIError(nil, errors.NewOpenShiftObjectNotFoundError("ID", "message"))
	require.Equal(t, http.StatusNotFound, httpStatus)
	require.NotNil(t, jerr.Code)
	require.Equal(t, jsonapi.ErrorCodeNotFound, *jerr.Code)
	require.NotNil(t, jerr.Status)
