...
# for the package "github.com/fabric8-services/fabric8-tenant/models" with coverage mode
...
test-all: prebuild-check test-unit test-integration test-remote test-with-minishift
...
test-integration-no-coverage: prebuild-check migrate-database $(SOURCES)
	$(call log-info,"Running test: $@")
	$(eval TEST_PACKAGES:=$(shell go list ./... | grep -v $(ALL_PKGS_EXCLUDE_PATTERN)))
	F8_DEVELOPER_MODE_ENABLED=1 F8_RESOURCE_DATABASE=1 F8_RESOURCE_UNIT_TEST=0 F8_POSTGRES_DATABASE=postgres go test -v -p 1 $(TEST_PACKAGES)
...
test-with-minishift: prebuild-check migrate-database
	$(call log-info,"Running test: $@")
	$(eval TEST_PACKAGES:=$(shell go list ./... | grep -v $(ALL_PKGS_EXCLUDE_PATTERN)))
	...
	go test -p 1 -v -run ".*Minishift.*" $(TEST_PACKAGES)
...
#  2. package name "github.com/fabric8-services/fabric8-tenant/model"
